...
public class BaseClass {
	...
	public synchronized void setup(String browser) throws IOException {
		...
		logger.info("WebDriver Initialized and Browser Maximized @ Size: " + getDriver().manage().window().getSize());
	...
	private synchronized void launchBrowser(String browser) {
		...
		if (seleniumGrid) {
			...
			if (browser.equalsIgnoreCase("chrome")) {
				...
				getDriver().manage().window().setSize(new Dimension(1920, 1080));// Ensure window size in headless mode
				ExtentManager.registerDriver(getDriver());
				logger.info("EdgeDriver Instance is created.");
			} else if (browser.equalsIgnoreCase("safari")) {
				// driver = new SafariDriver();
				// Safari does not support headless mode as of now
				throw new IllegalArgumentException("Safari browser does not support headless mode and only works on MacOS.");
			}
	...
	// The synchronized keyword ensures that the method is thread-safe, meaning only
	// one thread can execute the method at a time for the same object. In the
	// context of the tearDown method, it prevents multiple threads from
	// simultaneously accessing and modifying shared resources like the driver and
	// actionDriver instances. This is crucial in a multi-threaded test execution
	// environment (e.g., parallel test runs) to avoid race conditions and ensure
	...
	public static ActionDriver getActionDriver() {
		...
		// would it be there any issue if I use "return actionDriver" instead of "return
		// actionDriver.get()" in getActionDriver() method?

		// actionDriver is a ThreadLocal object, which means it is designed to store
		// values specific to the current thread. Using actionDriver.get() retrieves the
		// value associated with the current thread. If you directly return
		// "actionDriver", you would be returning the ThreadLocal object itself, not the
		// thread-specific value it holds. This would break the intended functionality
		// and could lead to runtime errors or incorrect behavior in a multi-threaded
		// environment.

		// In the code, actionDriver is a ThreadLocal<ActionDriver> object. The get()
		// method of ThreadLocal is used to retrieve the value stored for the current
		// thread. This ensures that each thread gets its own instance of ActionDriver,
